...
ClientConfig loadClient(const char configFile[]) {
    ...
    for (int i = 0; i < config.linkCount; i++) {
        config.links[i].sourceAddress = reader.Get(string("link") + to_string(i), "SourceAddress", "0.0.0.0");
        config.links[i].destinationAddress = reader.Get(string("link") + to_string(i), "DestinationAddress", "127.0.0.1");
        config.links[i].sourcePort = (int) reader.GetInteger(string("link") + to_string(i), "SourcePort", 6241);
        config.links[i].priority = (int) reader.GetInteger(string("link") + to_string(i), "Priority", 0);
    }
    return config;
